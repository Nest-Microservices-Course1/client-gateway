...
interface EnvVars {
  ...
  HOST_ORDER_MS: string;
  PORT_ORDER_MS: number;
}
...
const envSchema = joi
  .object({
    ...
    HOST_ORDER_MS: joi.string().required(),
    PORT_ORDER_MS: joi.number().required(),
  })
  .unknown(true);
...
export const envs = {
  ...
  hostOrderMs: envVars.HOST_ORDER_MS,
  portOrderMs: envVars.PORT_ORDER_MS,
};
